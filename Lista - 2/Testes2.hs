...
-- Turma: T01
{-
    Observações :  A fim de utilizar o mesmo banco de dados, funçoes auxiliares( que não utilizam ListComprehenssion ) e refatoramento de codigo
...
-- QUESTAO B
{-
atualizaEndSusL1 :: CPF -> CadastroSUS -> Endereco -> CadastroSUS
atualizaEndSusL1 cpf cadSus endereco 
...
atualizaEndSus cpf (x:xs) endereco 
    | cpf == getCPF x                   = xs ++ [ cidadaoAtualizado ]
    | otherwise                         = x: atualizaEndSus cpf xs endereco
    where
